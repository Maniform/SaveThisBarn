...
var speed_factor = Vector3(1, 1, 1)

func _init(_speed_factor:= speed_factor as Vector3):
...
func _ready():
	...
	speed.x = randf_range(-500, 500) * speed_factor.x
	speed.y = randf_range(-300, -1000) * speed_factor.y
	angle_speed = randf_range(-1, 1) * speed_factor.z
